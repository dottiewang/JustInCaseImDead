...
async function send(cus, key, idem) {
  const t = EMAILS[key];
  ...
  const r = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + env('RESEND_API_KEY'), 'Content-Type': 'application/json', ...(idem ? { 'Idempotency-Key': idem } : {}) },
    body: JSON.stringify({
  ...
  try {
    await query(
      `INSERT INTO sent_emails (stripe_customer_id, recipient_email, email_key, subject, resend_id)
       VALUES ($1, $2, $3, $4, $5)`,
      [cus.id, cus.email, key, fill(t.subject), result.id || null],
    );
  } catch (e) {
    console.error('Unable to record sent email:', e.message);
  }
}
